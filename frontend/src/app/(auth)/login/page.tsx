...
export default function LoginPage() {
  ...
  return (
    <div data-testid="login-page" className="relative min-h-[85vh] flex items-center justify-center px-4 overflow-hidden">
      ...
      <div className="w-full max-w-md relative animate-rise">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground mt-1.5">Bine ai revenit la Casa Pâinii</p>
        </div>

        <div className="bg-card border border-border/60 rounded-3xl p-7 soft-shadow-lg">
          {error && (
            <div data-testid="login-error" className="bg-destructive/10 border border-destructive/30 text-destructive px-4 py-3 rounded-2xl text-sm mb-5">
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleLogin} className="flex flex-col gap-4">
            ...
            <div>
              ...
              <div className="text-right mt-1.5">
                <Link href="/reset-password" className="text-xs text-primary hover:underline">Ai uitat parola?</Link>
              </div>
            </div>
        ...
        <p className="text-center text-sm text-muted-foreground mt-5">
          Nu ai cont?{' '}
          <Link href="/signup" data-testid="login-signup-link" className="text-primary hover:underline font-medium">Înregistrează-te</Link>
        </p>
      </div>
    </div>
  )
